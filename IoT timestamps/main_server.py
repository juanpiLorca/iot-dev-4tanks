...
class QuadrupleTanks_Namespace:
    def __init__(self, objects, idx, server):
        ...
        flags = thickener.add_object(self.idx, 'Flags')
        ...
        flag2stop = flags.add_variable(self.idx, 'flag2stop', 0)
        flag2start = flags.add_variable(self.idx, 'flag2start', 0)
        u_1.set_writable()
        u_2.set_writable()
        incomming_pkg.set_writable()
        flag2stop.set_writable()
        flag2start.set_writable()
...
class OPC_server:
    ...
    def start(self):
        self.server.start()
        print("###################### Quadruple Tank server runnning ######################")
...
if __name__ == '__main__':
    ...
    server.new_namespace(uri='QuadrupleTank', namespace=QuadrupleTanks_Namespace, name='QuadrupleTank_namespace')
    server.start()
