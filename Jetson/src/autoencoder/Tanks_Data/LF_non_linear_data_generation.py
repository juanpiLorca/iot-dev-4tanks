...
def generate_pattern(n_seqs=500, seqlen=60, window=20):
    ...
    for n in range(n_seqs):
        u1_val = random.uniform(0, 1)
        u2_val = random.uniform(0, 1)
...
if __name__ == '__main__':
    ...
    ## Input generation
    inputs = generate_pattern(n_seqs=100, seqlen=1000, window=5) 
    inputs *= 0.6
    ...
    for i in range(2):
        ...
        plt.plot(inputs[:, i], label=f'u_{i}')
    plt.title('Inputs')
    plt.legend()
    plt.grid()
    # plt.show()
    ...
    print(data_to_save.shape)
    np.save('No_Noised_Inputs/LF_data_NL_clean.npy', data_to_save)
    # np.save('data_noise.npy', np.concatenate([inputs_noise, series_noise], axis=1))
    torch.save(data_to_save, 'No_Noised_Inputs/LF_data_NL_clean.pkl')
